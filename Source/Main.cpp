...
#include <time.h>           // Required for: rand(), srand()
#include <Windows.h>          // Required for: delay()
...
void Start()
{
	...
	// Init window
	state.window = SDL_CreateWindow("Arcade Lovers Game", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN);
	...
	state.ship = SDL_CreateTextureFromSurface(state.renderer, IMG_Load("Assets/Heroe.png"));
	state.shot = SDL_CreateTextureFromSurface(state.renderer, IMG_Load("Assets/shot.png"));
	state.enemy = SDL_CreateTextureFromSurface(state.renderer, IMG_Load("Assets/Enemy.png"));
	state.score = SDL_CreateTextureFromSurface(state.renderer, IMG_Load("Assets/score.png"));

	SDL_QueryTexture(state.background, NULL, NULL, &state.background_height, NULL);
	...
	state.ship_x = 300;
	state.ship_y = SCREEN_HEIGHT / 1.15;
	state.last_shot = 0;
	state.scroll = 0;
	state.enemy_x = 310;
	state.enemy_y = 0;
...
void Finish()
{
	...
	SDL_DestroyTexture(state.enemy);
	SDL_DestroyTexture(state.score);
	IMG_Quit();
...
void MoveStuff()
{
	...
	if (state.ship_x > SCREEN_WIDTH) state.ship_x = 0;
	else if (state.ship_x < 0) state.ship_x = SCREEN_WIDTH;

	if (state.ship_y > SCREEN_HEIGHT-70) state.ship_y = SCREEN_HEIGHT-70;
	else if (state.ship_y < 0) state.ship_y = 0;

	if (state.enemy_y > SCREEN_HEIGHT)
	{
		state.enemy_y = 0;
		state.enemy_x = rand() % (SCREEN_WIDTH-20);
	}
	...
	{
		if (state.last_shot == MAX_SHIP_SHOTS) state.last_shot = 1;

		state.shots[state.last_shot].alive = true;
		state.shots[state.last_shot].x = state.ship_x + 44;
		state.shots[state.last_shot].y = state.ship_y + 0;
		state.last_shot++;
	...
	if (state.keyboard[SDL_SCANCODE_SPACE] == KEY_DOWN)
	{
		...
		state.shots[state.last_shot].alive = true;
		state.shots[state.last_shot].x = state.ship_x - 13;
		state.shots[state.last_shot].y = state.ship_y + 0;
		state.last_shot++;
	...
	if (state.enemy_y == state.shots[MAX_SHIP_SHOTS].y && state.enemy_x == state.shots[MAX_SHIP_SHOTS].x)
	{
		state.enemy_y = 0;
		state.enemy_x = rand() % (SCREEN_WIDTH - 20);
		state.enemy_live++;
...
void Draw()
{
	...
	rec.x = state.enemy_x; rec.y = state.enemy_y; rec.w = 64; rec.h = 64;
	SDL_RenderCopy(state.renderer, state.enemy, NULL, &rec);
